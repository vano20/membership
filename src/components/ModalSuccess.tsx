...
export default function ModalSuccess({ 
  ...
  const footer = (
    <div className="flex gap-4">
      <Button onClick={onClose} variant="secondary" rounded>
        Close
      </Button>
    </div>
  );
  ...
  const modal = (
    <Modal onClose={onClose} footer={footer}>
      <div className="flex flex-col items-start gap-4 text-wrap break-words">
        <h1 className="text-3xl font-semibold">Registrasi berhasil !</h1>
        <p>
          Silahkan menghubungi admin terkait pembayaran pada nomor whatsapp
          berikut
        </p>
        <p>
          <a
            href={encodeURI(wa)}
            target="_blank"
            className="ml-1 text-blue-600 hover:text-blue-500/30 flex justify-center items-center"
