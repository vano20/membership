...
export default function AdminPage() {
  ...
  const { data: { data, meta: respMeta } = {}, isFetching } =
    useFetchListRegistrationQuery({ ...meta, ...filter, npwp });
  const [updateStatus, { isLoading }] = useUpdateRegistrationsStatusMutation();
  const { data: detailRegistration, isError } = useFetchRegistrationQuery(
    item,
    {
      skip: !Object.keys(item).length && !showModal,
    }
  );
  ...
  const encodePhone = (val) => {
    ...
    if (!isLeadingPlus)
      formattedPhone = `+62${formattedPhone.slice(isLeadingZero ? 1 : 0)}`;
  ...
  const headers = [
    ...
    },
    {
      ...
      render: (item) => (
        <a
          href={encodePhone(item.phone_number)}
          target="_blank"
          className="text-blue-600 hover:text-blue-500/30 flex justify-center items-center"
        >
    ...
    },
    {
      ...
      render: (item) => (
        <div className="flex items-center justify-end gap-4">
          ...
          <Button rounded onClick={() => handleShowModal(item)}>
            Approve
          </Button>
          <Button
            rounded
  ...
  useEffect(() => {
    if (detailRegistration && !isError) {
      setMembershipId(detailRegistration?.membership_id || "");
      setDetail(detailRegistration);
    }
  }, [detailRegistration, isError]);

  useEffect(() => {
    setMetaData({
      ...metaData,
      last_page: respMeta?.last_page || meta.last_page,
      total: respMeta?.total || meta.total,
      from: respMeta?.from || meta.from,
      to: respMeta?.to || meta.to,
    });
  }, [respMeta, meta, metaData]);
  ...
  const handleSubmit = async (status) => {
    try {
      ...
      toast.error(error.message || "Terjadi kesalahan saat approval.");
    }
  };
  ...
  const footer = (
    <div className="flex gap-4">
      ...
      <Button rounded isDisabled={isLoading} onClick={() => handleSubmit()}>
        Save
      </Button>
      ...
      <div className="md:p-0 text-wrap break-words py-4">
        ...
        <div className="flex flex-col gap-2">
          <div className="flex gap-2 p-2 w-fit">
            ...
            {detail?.contact_person}, {detail?.position}
          </div>
          <div className="flex gap-2 p-2 w-fit">
        ...
        <div className="p-2 text-lg my-4">
          ...
          sebagai perusahaan&nbsp;
          <span className="font-semibold">{detail.qualification}</span>
        </div>
        <input
  ...
  return (
    <>
      <div className="flex flex-col w-full h-full bg-white border-2 rounded-md shadow-lg shadow-slate-300 mx-auto p-8 border-transparent">
        <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-4">
          <h2 className="md:mb-0 text-3xl font-semibold mb-4">Approval</h2>

          <div className="flex flex-col md:flex-row md:items-center gap-4">
            <FilterAdminTable
              onClickSearch={handleUpdateFilter}
              onPressEnter={handleUpdateFilter}
            />
      ...
      <ModalConfirm
        ...
        desc={`Hapus registrasi dari ${item.npwp} - ${item.company_name}`}
        handleChange={(val) => setShowModalConfirm(val)}
